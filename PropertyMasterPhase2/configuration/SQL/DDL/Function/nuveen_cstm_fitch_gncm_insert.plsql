CREATE OR REPLACE PROCEDURE nuveen_cstm_fitch_gncm_insert()
 LANGUAGE plpgsql
AS $procedure$
BEGIN
...
'GNCM_FITCH_INSERT' as last_chg_usr_id,
'ENGLISH' as nls_cde,
'Yes' as cmnt_txt,
null as end_tms,
'ACTIVE' as cmnt_stat_typ,
new_oid() as gncm_oid ,
'FITCH' as data_src_id,
...
UPDATE FT_T_GNCM SET END_TMS= CURRENT_TIMESTAMP,last_chg_usr_id='FITCH_SOFT_DELETE' WHERE cross_ref_id in(
...
AND GNCM.end_tms IS null and gncm_tbl_typ = 'ISRT'  AND isrt.data_src_id = 'FITCH'));
...
AND GNCM.end_tms IS null and gncm_tbl_typ = 'RTOP'  AND rtop.data_src_id = 'FITCH'));
